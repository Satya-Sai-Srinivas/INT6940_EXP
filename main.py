...
def export_table_csv(
    ...
    Optionally filter by a date column for today's records.
    """
    try:
...
app = FastAPI(title="Patient Check-in Dashboard API")
...
def clinicians_overview(db=Depends(get_db)):
    ...
    try:
        ...
        """))
        clinicians = [dict(row._mapping) for row in result]  # <- Fix applied here
